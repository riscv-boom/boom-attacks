...
#define CODE_MEM_START 0x80000000
...
int main(void){
    ...
    uint8_t stored_output[32] = {0};

    // try to read out the secret
    for(uint64_t len = 0; len < 0x30000; ++len){
        ...
        printf("%x ", output[0]); 
        stored_output[len % 32] = output[0];

        if ( len % 32 == 0 ){
            printf("-- ");
            for( uint16_t iter = 0; iter < 32; ++iter){
                printf("%c ", stored_output[iter]);
            }
            printf("\n");
        }
